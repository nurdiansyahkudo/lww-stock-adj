...
class StockAdj(models.Model):
    ...
    def _compute_sn_duplicated(self):
        ...
        quants_with_duplicated_sn = self.env['stock.adj'].search([('lot_id', 'in', duplicated_sn_ids)])
    ...
    def _set_inventory_quantity(self):
        ...
        if not self._is_inventory_mode():
            return
        quant_to_inventory = self.env['stock.adj']
        for quant in self:
    ...
    def _search_on_hand(self, operator, value):
        ...
        quant_query = self.env['stock.adj']._search(domain_loc)
    ...
    def _load_records_create(self, values):
        ...
        return super(StockAdj, self.with_context(inventory_mode=True))._load_records_create(values)

    def _load_records_write(self, values):
        """ Only allowed fields should be modified """
        return super(StockAdj, self.with_context(inventory_mode=True))._load_records_write(values)
    ...
    def write(self, vals):
        ...
        return super(StockAdj, self).write(vals)
    ...
    def action_view_adjustment(self):
        ...
        view_id = self.env.ref('view_stock_adj_tree_inventory_editable').id
        action = {
    ...
    def action_apply_all(self):
        quant_ids = self.env['stock.adj'].search(self.env.context['active_domain']).ids
    ...
    def _get_quants_by_products_locations(self, product_ids, location_ids, extra_domain=False):
        res = defaultdict(lambda: self.env['stock.adj'])
        if product_ids and location_ids:
            ...
            needed_quants = self.env['stock.adj']._read_group(
                domain,
    ...
    def _onchange_serial_number(self):
        if self.lot_id and self.product_id.tracking == 'serial':
            message, dummy = self.env['stock.adj'].sudo()._check_serial_number(self.product_id,
                                                                                 self.lot_id,
                                                                                 self.company_id)
            if message:
    ...
    def _onchange_product_id(self):
        ...
        if self.product_id.tracking in ['lot', 'serial']:
            previous_quants = self.env['stock.adj'].search([
    ...
    def _unlink_zero_quants(self):
        ...
        quants = self.env['stock.adj'].browse([quant['id'] for quant in self.env.cr.dictfetchall()])
        quants.sudo().unlink()
    ...
    def _check_serial_number(self, product_id, lot_id, company_id, source_location_id=None, ref_doc_location_id=None):
        ...
        if product_id.tracking == 'serial':
            ...
            quants = self.env['stock.adj'].search([('product_id', '=', product_id.id),
                                                     ('lot_id', '=', lot_id.id),
                                                     ('quantity', '!=', 0),
